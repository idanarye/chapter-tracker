...
impl MediaTypesActor {
    fn add_new_entry_row(&mut self, ctx: &mut actix::Context<Self>) {
        ...
        self.widgets.lst_media_types.add(&entry.widgets.row_media_type);
        ctx.spawn(
            entry.add_verifications_to_edit_mode(
                EditMode::builder()
            ...
            .then(move |user_saved, actor, _| {
                async move {
                    if let Some(media_type_id) = user_saved {
                        let query = sqlx::query_as("SELECT * FROM media_types WHERE id = ?").bind(media_type_id);
        ...
        .then(|result, actor, ctx| {
            if let Some(result) = result {
                let mut row = actor.media_types.remove(&-1).expect("entry with id=-1 should have been in media_types");
                row.model = result;
                row.update_widgets_from_model();
                actor.media_types.insert(row.model.id, row);
                actor.main_app.do_send(crate::gui::msgs::UpdateMediaTypesList);
                actor.add_new_entry_row(ctx);
            }
            futures::future::ready(())
        })
...
impl MediaTypeRow {
    ...
    fn add_verifications_to_edit_mode(&self, edit_mode: EditMode) -> EditMode {
        edit_mode
            .with_edit_widget(self.widgets.txt_media_type_name.clone(), "changed", self.model.name.clone(), |text| {
                if text == "" {
...
impl actix::Handler<woab::Signal<i64>> for MediaTypesActor {
    ...
    fn handle(&mut self, msg: woab::Signal<i64>, ctx: &mut Self::Context) -> Self::Result {
        ...
        Ok(match msg.name() {
            ...
            "edit_media_type" => {
                ctx.spawn(
                    media_type.add_verifications_to_edit_mode(
                        EditMode::builder()
                        .stack(media_type.widgets.stk_media_type_edit.clone())
                        .save_button(media_type.widgets.btn_save_media_type.clone())
                        .cancel_button(media_type.widgets.btn_cancel_media_type_edit.clone())
                        .build()
                    )
                    ...
                    .then(move |_, actor, ctx| {
                        crate::actors::DbActor::from_registry().send(crate::msgs::RefreshList {
                            orig_ids: Default::default(),
                            query: sqlx::query_as("SELECT * FROM media_types WHERE id = ?").bind(media_type_id),
                            id_dlg: |row_data: &models::MediaType| row_data.id,
                            addr: ctx.address(),
                        }).into_actor(actor)
                        .then(move |_, _, _| {
            ...
            "delete_media_type" => {
                ...
                ctx.spawn(async move {
                    let mut con = db::request_connection().await.unwrap();
                    let (num_serieses,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM serieses WHERE media_type = ?")
                        .bind(media_type_id)
                        .fetch_one(&mut con)
                        .await.unwrap();
                    if 0 < num_serieses {
                        woab::run_dialog(&gtk::MessageDialog::new::<gtk::Window>(
                                None,
                                gtk::DialogFlags::MODAL,
                                gtk::MessageType::Error,
                                gtk::ButtonsType::Close,
                                &format!("Cannot delete {:?} - {} serieses are using it", media_type_name, num_serieses),
                        ), true).await;
                        false
                    } else {
                        let user_decision = woab::run_dialog(&gtk::MessageDialog::new::<gtk::ApplicationWindow>(
                                None,
                                gtk::DialogFlags::MODAL,
                        ...
                        if user_decision == gtk::ResponseType::Yes {
                            sqlx::query("DELETE FROM media_types WHERE id = ?").bind(media_type_id).execute(&mut con).await.unwrap();
                            true
                        } else {
                            false
                        }
                ...
                .then(move |did_delete, actor, _ctx| {
                    if did_delete {
                        actor.widgets.lst_media_types.remove(&actor.media_types[&media_type_id].widgets.row_media_type);
                        actor.main_app.do_send(crate::gui::msgs::UpdateMediaTypesList);
                    }
                    futures::future::ready(())
                }));
